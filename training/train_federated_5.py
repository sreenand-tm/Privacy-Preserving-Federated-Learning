"""
Phase B: Federated Learning Simulation.

Simulates 5 clients collaboratively training a global model using a Non-IID 
(Dirichlet) split of the UrbanSound8K dataset. 

Compares:
1. Standard FedAvg (Clean)
2. Standard FedAvg (With 1 Malicious Attacker)
3. Proposed Method: FedProx + Krum Aggregation + Differential Privacy (With 1 Malicious Attacker)
"""
import os
import sys
# Ensure imports work when run from inside the fed/ folder
...
RESULTS_DIR = os.path.join(PROJECT_ROOT, "fed")
os.makedirs(RESULTS_DIR, exist_ok=True)
CHECKPOINT_FILE = os.path.join(RESULTS_DIR, "fl_checkpoint.pth")
...
def aggregate_krum(updates, f=1, verbose=False):
    ...
    # Only use floating-point tensors for distance computation
    flat_updates = []
    for u in updates:
...
def plot_results(fedavg_clean, fedavg_atk, robust_atk):
    ...
    plt.title(f"Federated Learning Accuracy (Non-IID, $\\alpha$={DIRICHLET_ALPHA})", fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "federated_comparison.png"), dpi=150)
    plt.close()
    print(f"\n[PLOT SAVED] Chart available at {os.path.join(RESULTS_DIR, 'federated_comparison.png')}")
...
def save_simulation_results(client_indices, accs_clean, accs_atk, accs_robust, 
    ...
    with open(os.path.join(RESULTS_DIR, "federated_accuracy_logs.csv"), "w") as f:
        ...
        for r in range(NUM_ROUNDS):
            f.write(f"{r+1},{accs_clean[r]:.2f},{accs_atk[r]:.2f},{accs_robust[r]:.2f}\n")
            
    print(f"All results successfully saved in: {RESULTS_DIR}")

# ─── Main Simulation ──────────────────────────────────────────────────────────
def main():
    print("=" * 60)
    print("  FEDERATED LEARNING SIMULATOR (PHASE B)")
    print("=" * 60)
    ...
    if os.path.exists(CHECKPOINT_FILE):
        ...
        # Use weights_only=False because we save a dict containing native python objects
        ...
        print("\n[NEW RUN] Creating non-IID data partitions...")
    ...
    # ---------------- RUN 2 ----------------
    if phase == 2:
        print("\n" + "-" * 50 + "\n  RUN 2: FedAvg (With Attacker)\n" + "-" * 50)
        for r in range(start_round, NUM_ROUNDS + 1):
            updates = [c.train(model_fedavg_atk, is_malicious=(c.id == 4)) for c in clients]
    ...
    # ---------------- RUN 3 ----------------
    if phase == 3:
        print("\n" + "-" * 50 + "\n  RUN 3: FedProx + Krum + DP (With Attacker)\n" + "-" * 50)
        for r in range(start_round, NUM_ROUNDS + 1):
            updates = [c.train(model_robust, use_prox=True, mu=0.01, use_dp=True, dp_epsilon=1.0, is_malicious=(c.id == 4)) for c in clients]
            verbose = (r <= 3)  # Show Krum diagnostics for first 3 rounds
    ...
    if len(accs_robust) == NUM_ROUNDS:
        ...
        # Cleanup checkpoint when successfully finished
        if os.path.exists(CHECKPOINT_FILE):
